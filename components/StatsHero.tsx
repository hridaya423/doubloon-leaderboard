...
const StatsHero = ({ stats }: StatsHeroProps) => {
  return (
        ...
        <div className="text-center relative">
          <div className="relative z-10 mb-8">
            <h1 className="text-8xl font-black tracking-tight mb-4">
              ...
              <span className="inline-block transform hover:scale-105 transition-transform duration-300">
                <span className="bg-gradient-to-r from-yellow-400 via-amber-300 to-yellow-400 bg-clip-text text-transparent
                  animate-gradient bg-[length:200%_auto] delay-75">
                  Leaderboard
                </span>
              </span>
            </h1>
            ...
            <div className="flex justify-center gap-6 mt-8">
              ...
              />
              <StatCard
                title="Average Doubloons"
                value={Math.round(stats.totalDoubloons / stats.totalUsers)}
                isLoading={stats.isLoading}
              />
            </div>
          </div>
...
const StatCard = ({ title, value, isLoading }: StatCardProps) => (
  <div className="px-6 py-3 bg-yellow-500/10 rounded-2xl backdrop-blur-sm border border-yellow-500/20
