      ...
      {!isSearching && (
        <div className="mt-8 flex items-center justify-between">
          <Button
            ...
            className="bg-white/5 hover:bg-yellow-500/20 text-yellow-400 border border-yellow-500/20
              px-6 py-3 rounded-xl disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Previous
          </Button>
          <span className="text-yellow-400/70 text-lg">
...
const LeaderboardCard = ({ user, getDisplayValue, onClick }: LeaderboardCardProps) => (
  <div
    onClick={() => onClick(user)}
    className="group bg-white/5 hover:bg-white/10 border border-yellow-500/10 hover:border-yellow-500/30
      rounded-2xl p-6 cursor-pointer transition-all duration-300 transform hover:scale-[1.02]"
  >
    <div className="flex items-center justify-between">
      <div className="flex items-center space-x-6">
        <div className="flex items-center justify-center w-16 h-16 bg-yellow-500/10 rounded-xl
          group-hover:bg-yellow-500/20 transition-colors">
          <span className="text-2xl font-bold text-yellow-500">#{user.rank}</span>
        </div>
        <div className="h-16 w-16 rounded-xl bg-white/10 overflow-hidden ring-2 ring-yellow-500/20
          group-hover:ring-yellow-500/40 transition-all transform group-hover:rotate-6">
          <img
        ...
        <div>
          <div className="flex items-center space-x-3">
            <span className="font-bold text-xl group-hover:text-yellow-400 transition-colors">
              {user.username}
            </span>
            <a
              href={`${user.slack}`}
              ...
              className="text-yellow-500/50 hover:text-yellow-400 transition-colors
                bg-yellow-500/10 p-2 hover:bg-yellow-500/20"
            >
      ...
      <div className="flex items-center space-x-3 bg-yellow-500/10 rounded-xl px-6 py-3
        group-hover:bg-yellow-500/20 transition-all">
